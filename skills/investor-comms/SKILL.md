---
name: investor-comms
description: "Keep every investor warm — including the ones who said no — through consistent monthly updates on two parallel communication tracks. Use this skill whenever the user wants to write an investor update or monthly update, keep investors warm, follow up with a fund that passed, or nurture their pipeline between milestones. Trigger it for monthly updates and any \"stay in touch with investors who passed\" request (it also points the way on announcing a closed raise). Consistency here — updating even when there's nothing dramatic to report — is what turns a \"no\" into a \"yes\" next round."
---

# Investor Comms

Most founders go quiet after a "no" and silent between milestones. That's the
mistake. The investors who pass this round are often the leads of your next one —
*if* they watch you execute for six months. Warmth is built by consistency, not
charisma. This skill runs that consistency.

Suggested cadence: monthly, to both tracks, same week every month.

This skill runs when invoked. Its cadence is a recommendation, not an installed
schedule. Use the project files and user-supplied updates; do not imply that email,
calendar or CRM activity has been synced unless an available, authorized
integration actually supplied it. Report the date and source of the latest data.

Prepare drafts by default. Generating a draft does not mean it was sent and must
not advance a thread to Contacted. Send or publish only when the user explicitly
requests it and an appropriate integration is available; record a sent date only
after confirmed delivery or a user-reported send.

Use the user's instructions and existing decisions as the authority. Read and write
raise artifacts in the user's project folder, not the installed skill directory.
In chat-only sessions, use the supplied files and return updated artifacts inline
or as downloads; do not claim a project file was saved unless it was.

## Prerequisites

- Read `pipeline.md` from the project folder to know who's on which track
  (live/yes vs. passed) and who owns each relationship. If it's missing, run
  `pipeline` first — you can't segment tracks without the state.
- Read `fundraising-plan.md` for the mode — in passive mode this skill IS the
  raise (converting warmth over time); in active mode it's the between-milestones
  layer. If missing, run `fundraising-plan` first.
- Read `raise-context.md` for the facts every update draws on.

## Two parallel tracks

### Track 1 — Investors who said YES (or are live)
They're committed or close. Updates keep momentum and make them useful (intros,
hires, customers). Tone: collaborative, ask for specific help.

### Track 2 — Investors who said NO (or not now)
They passed but didn't dislike you. The goal is to make the next "yes" inevitable
by letting them watch you hit the milestones you said you would. Tone: confident,
no neediness, no re-pitching. Every update that lands a promised milestone quietly
rebuilds the case. When you out-execute the reason they passed, *they* re-engage.

## The monthly update (the core artefact)

Short, skimmable, sent like clockwork. Structure:

```
[Company] — [Month] Update
TL;DR: [one line — the month in a sentence]
📈 Metrics: [3–5 numbers vs. last month — the same metrics every time]
✅ Wins: [2–4, concrete]
🚧 Challenges: [1–2, stated honestly — this is what builds trust]
🙏 Asks: [specific — intros to X, hiring Y, anyone who knows Z]
What's next: [the next milestone]
```

The honest "challenges" line is the most important. Updates that are all wins read
as PR and get skipped. Naming a real challenge — and later showing you solved it —
is what makes investors trust the wins.

## Announcing the raise

When the round closes: lead with the mission not the money, write it in the
founders' own voices (§13), keep the facts reconciled with the deck and room,
and never announce before the leads are ready — draft it then as ordinary
writing work, coordinated with them.

## Procedure

1. Read `pipeline.md` and identify the track (yes/live vs. passed) for each
   recipient.
2. For updates: pull the same recurring metrics each month so progress is legible;
   write the honest-challenge line; make the asks specific.
3. Keep a consistent cadence — the value is in showing up every month, on time.

## Principles

- **Consistency is the strategy.** A mediocre update sent every month beats a
  brilliant one sent twice. The signal is reliability.
- **Honesty in the challenges line.** Trust is built by what you admit, not what
  you boast. The challenge you name and then solve is your strongest proof.
- **A "no" is a "not yet with this information."** Out-execute the reason they
  passed, in writing, monthly, and the next round writes itself.
- **Mission before money in public.** The raise is a means; lead with what it's for.
